...
class Norma2Test extends munit.FunSuite {
  ...
  test("Norma2 boost to Norma3 by stars objective") {
    player.increaseStars(200)
    player.changeRoad(1)
    assert(norma.checkBoost(player).isInstanceOf[Norma3])
  }

  test("Norma2 boost to Norma3 by victories objective") {
    player.increaseVictories(14)
    player.changeRoad(2)
    assert(norma.checkBoost(player).isInstanceOf[Norma3])
  }

  test("Norma2 don't boost to Norma3 by stars objective") {
    player.changeRoad(1)
    assert(norma.checkBoost(player).isInstanceOf[Norma2])
  }

  test("Norma2 don't boost to Norma3 by victories objective") {
    player.changeRoad(2)
    assert(norma.checkBoost(player).isInstanceOf[Norma2])
  }
